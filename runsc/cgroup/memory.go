...
func (s *MemoryGroup) Apply(d *cgroupData) (err error) {
	...
	log.Debugf("Apply memory group: path %s", path)
...
func setKernelMemory(path string, kernelMemoryLimit int64) error {
	...
	if !PathExists(filepath.Join(path, cgroupKernelMemoryLimit)) {
...
func setMemoryAndSwap(path string, cgroup *Cgroup) error {
	// If the memory update is set to -1 we should also
	// set swap to -1, it means unlimited memory.
	if cgroup.Resources.Memory == -1 {
		// Only set swap if it's enabled in kernel
		if PathExists(filepath.Join(path, cgroupMemorySwapLimit)) {
...
func (s *MemoryGroup) Set(path string, cgroup *Cgroup) error {
...
func (s *MemoryGroup) GetStats(path string, stats *Stats) error {
...
func memoryAssigned(cgroup *Cgroup) bool {
